...
{ 
        ...
        public override void Update()
        {
            ...
            if (SnapsEngine.GetFireGamepad())
                foreach (MissileSprite missile in Missiles)
                    if (!missile.IsFlying)
                        missile.FireMissle(spriteValue.CenterX, spriteValue.CenterY);
        }
    }
    ...
    public class LineAlien : AlienSprite
    {
        ...
        public LineAlien(ImageSprite sprite, double xSpeed, double ySpeed,
            RocketSprite target, double xMax, double xMin) :
    ...
    public class MissileSprite : MovingSprite
    {
        ...
        public MissileSprite(ImageSprite sprite, RocketSprite rocket, double xSpeed,
            double ySpeed, List<AlienSprite> aliens ) : base(sprite: sprite, xSpeed: xSpeed, ySpeed: ySpeed)
        {
            rocketValue = rocket;
            aliensValue = aliens;
            spriteValue.Hide();
        ...
        public void FireMissle(double missileX, double missileY)
        {
            if (IsFlying)
                return;
    ...
    public void StartProgram()
    {
        ...
        for (int i = 0; i < huntingParty; i = i+1)
        {
            double xRandPos = Rand.Next(minValue: 0, maxValue: ((int)SnapsEngine.GameViewportWidth));
            double yRandPos = Rand.Next(minValue: 0, maxValue: ((int)SnapsEngine.GameViewportHeight));
            ...
            chasingAlienImage.Left = xRandPos;
            chasingAlienImage.Top = yRandPos;
        ...
        for (int i = 0; i < noOfAliens; i = i+1)
        {
            ...
            LineAlien alien = new LineAlien(sprite: alienImage, xSpeed: 2, ySpeed: 0,
                target: rocket, xMax: xMax, xMin: xMin);
            sprites.Add(alien);
            aliens.Add(alien);            
        ...
        MissileSprite missle = new MissileSprite(sprite: missileImage, rocket: rocket, xSpeed: 0,
            ySpeed: -15, aliens: aliens);
        sprites.Add(missle);

        rocket.Missiles.Add(missle);
